...
class Morfy
{
    ...
    {
        if ($config['templater']) {
            require LIBRARIES_PATH . '/Templater/' . $config['templater'] . '.php';
        } else {
    ...
    public function generateToken($new = false)
    {
        ...
        if (isset($_SESSION[(string) Morfy::$security_token_name])) {
            $token = $_SESSION[(string) Morfy::$security_token_name];
        } else {
            $token = null;
        }
